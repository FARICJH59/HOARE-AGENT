...
from hoare_engine.aesirgrid_authority import AuthorityLease, AuthorityStatus, ControlledActionRequest, admit_controlled_action
...
def test_live_mode_never_becomes_implicitly_allowed_by_a_controlled_lease():
    product = _staged_product()
    lease = AuthorityLease(lease_id="lease-aesirgrid-controlled", tenant_id="tenant-grid-001", product_id=product.product_id, action="apply_maintenance_setpoint", mode=AesirGridMode.CONTROLLED, issued_at_s=100.0, expires_at_s=200.0)
    result = admit_controlled_action(ControlledActionRequest(tenant_id="tenant-grid-001", product_id=product.product_id, action="apply_maintenance_setpoint", requested_mode=AesirGridMode.LIVE, now_s=150.0, lease=lease))
