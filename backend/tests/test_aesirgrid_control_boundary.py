...
from hoare_engine.aesirgrid_authority import AuthorityLease, ControlledActionRequest
...
def test_denied_admission_never_invokes_existing_executor():
    calls = []
    result = execute_governed_control(_request(None), lambda: calls.append("executed"))
...
def test_valid_admission_invokes_executor_once():
    calls = []
    result = execute_governed_control(_request(_valid_lease()), lambda: calls.append("executed") or {"status": "accepted"})
...
def test_expired_lease_blocks_existing_executor():
    ...
    expired = AuthorityLease(
        lease_id=lease.lease_id, tenant_id=lease.tenant_id, product_id=lease.product_id,
        action=lease.action, mode=lease.mode, issued_at_s=lease.issued_at_s, expires_at_s=150.0,
    )
    result = execute_governed_control(_request(expired), lambda: calls.append("executed"))
