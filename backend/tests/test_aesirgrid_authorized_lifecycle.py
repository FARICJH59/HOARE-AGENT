...
def _request(lease=None):
    return ControlledActionRequest(tenant_id="tenant-grid-001", product_id="aesirgrid-predictive-maintenance", action="apply_maintenance_setpoint", requested_mode=AesirGridMode.CONTROLLED, now_s=150.0, lease=lease)
...
def test_valid_lease_permits_explicit_staged_to_authorized_transition():
    product = _staged_product()
    lease = AuthorityLease(lease_id="lease-aesirgrid-001", tenant_id="tenant-grid-001", product_id=product.product_id, action="apply_maintenance_setpoint", mode=AesirGridMode.CONTROLLED, issued_at_s=100.0, expires_at_s=200.0, status=AuthorityStatus.VALID)
...
def test_authorization_does_not_grant_product_execution_authority():
    product = _staged_product()
    lease = AuthorityLease(lease_id="lease-aesirgrid-002", tenant_id="tenant-grid-001", product_id=product.product_id, action="apply_maintenance_setpoint", mode=AesirGridMode.CONTROLLED, issued_at_s=100.0, expires_at_s=200.0, status=AuthorityStatus.VALID)
    admission = admit_controlled_action(_request(lease))
    authorized = authorize_product(product, admission)
