...
def _lease(**overrides):
    values = dict(lease_id="lease-aesirgrid-001", tenant_id="tenant-grid-001", product_id="aesirgrid-predictive-maintenance", action="apply_maintenance_setpoint", mode=AesirGridMode.CONTROLLED, issued_at_s=100.0, expires_at_s=200.0, status=AuthorityStatus.VALID)
    values.update(overrides)
    return AuthorityLease(**values)


def _request(**overrides):
    values = dict(tenant_id="tenant-grid-001", product_id="aesirgrid-predictive-maintenance", action="apply_maintenance_setpoint", requested_mode=AesirGridMode.CONTROLLED, now_s=150.0, lease=_lease())
    values.update(overrides)
...
def test_mode_mismatch_is_denied():
    result = admit_controlled_action(_request(requested_mode=AesirGridMode.LIVE))
    assert result.decision is AegisDecision.DENY


def test_missing_lease_escalates():
