...
def _build_staged_product():
    product = build_product_definition(product_id="aesirgrid-predictive-maintenance", product_version="1.0.0", domain="energy", capabilities=("Telemetry", "PredictiveMaintenance", "AssetHealthScoring", "AnomalyDetection"), domain_policies=("energy.grid.safety.v1",), evidence_requirements=("telemetry-integrity", "model-verification", "AEGIS-authorization"), deployment_profiles=("simulation", "shadow", "controlled", "live"), vertical_ip_refs=("aesirgrid:grid-models:v1",), customer_ip_refs=("customer:grid-operator:telemetry:v1",), metadata={"intent": "Build an energy-grid predictive-maintenance system for AesirGrid using telemetry from grid assets.", "case_study": "HOARE-CS-001"})
...
def test_aesirgrid_intent_to_shadow_to_controlled_admission():
    ...
    telemetry = SyntheticGridTelemetryProvider([GridTelemetry("substation-001", 58.0, 2.0, 72.0, 60.01, 1000.0), GridTelemetry("substation-002", 104.0, 9.0, 94.0, 59.70, 1000.0)])
    ...
    calls = []
    lease = AuthorityLease(lease_id="lease-aesirgrid-e2e-001", tenant_id="tenant-grid-001", product_id=product.product_id, action="apply_maintenance_setpoint", mode=AesirGridMode.CONTROLLED, issued_at_s=100.0, expires_at_s=200.0, status=AuthorityStatus.VALID)

    escalated = execute_governed(_controlled_request(product), lambda: calls.append("executed"), product=product)
    ...
    staged_execution = execute_governed(_controlled_request(product, lease), lambda: calls.append("executed") or "should-not-run", product=product)
    ...
    executed = execute_governed(_controlled_request(authorized_product, lease), lambda: calls.append("executed") or "executor-result", product=authorized_product)
