...
from hoare_engine.aesirgrid_authority import AuthorityLease, ControlledActionRequest, admit_controlled_action, authorize_product
...
def _staged_product():
    product = build_product_definition(product_id=PRODUCT_ID, product_version="1.0.0", domain="energy", capabilities=("Telemetry", "PredictiveMaintenance", "AssetHealthScoring", "AnomalyDetection"), domain_policies=("energy.grid.safety.v1",), workflows=("telemetry-ingestion", "asset-health-assessment"), integrations=("grid-telemetry",), deployment_profiles=("simulation", "shadow", "controlled", "live"), compliance_profiles=("NIST-AI-RMF",), evidence_requirements=("telemetry-integrity", "model-verification", "AEGIS-authorization"), vertical_ip_refs=("aesirgrid:grid-models:v1",), customer_ip_refs=("customer:grid-operator:telemetry:v1",), metadata={"case_study": "HOARE-CS-001"})
...
def _lease():
    return AuthorityLease(lease_id="lease-aesirgrid-e2e-001", tenant_id=TENANT_ID, product_id=PRODUCT_ID, mode=AesirGridMode.CONTROLLED, issued_at_s=100.0, expires_at_s=200.0)


def _request(lease=None):
    return ControlledActionRequest(tenant_id=TENANT_ID, product_id=PRODUCT_ID, action="apply_maintenance_setpoint", requested_mode=AesirGridMode.CONTROLLED, now_s=150.0, lease=lease)
...
def test_complete_case_study_reaches_authorized_existing_executor():
    ...
    provider = SyntheticGridTelemetryProvider([GridTelemetry(asset_id="substation-001", temperature_c=58.0, vibration_mm_s=2.0, load_pct=72.0, frequency_hz=60.01, timestamp_s=1000.0)])
    ...
    calls = []
    result = execute_governed_control(request, lambda: calls.append("physical-operation") or "executor-ok")
...
def test_case_study_rejects_tenant_mismatch_before_executor():
    ...
    request = ControlledActionRequest(tenant_id="different-tenant", product_id=base.product_id, action=base.action, requested_mode=base.requested_mode, now_s=base.now_s, lease=base.lease)
